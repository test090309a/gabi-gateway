            ...
            if intent == "som_search":
                ...
                if result and result.get("success"):
                    ...
                    if raw_results:
                        ...
                        # Modell-Auswahl basierend auf Query-Typ
                        ...
                        if is_code:
                            extraction_model = "qwen2.5-coder:7b" if "qwen2.5-coder:7b" in available else "llama3.1:8b"
                        elif is_weather:
                            extraction_model = "qwen2.5:7b" if "qwen2.5:7b" in available else "llama3.1:8b"
                        else:
                            extraction_model = _pick_best_model(available, min_size=7.0) or DEFAULT_MODEL
                        
                        _progress_set_active_model(request_id, extraction_model)
                        
                        search_context = ""
                        for i, res in enumerate(raw_results[:6], 1):
...
**ANTWORT (nur basierend auf den Quellen):**"""
                ...
                if url:
                    logger.info(f"🌐 SoM Navigation: {url}")
                ...
                if not learned:
                    return {
                        "status": "success",
                        "reply": "📭 Ich habe noch nichts gelernt.",
                        "request_id": request_id
                    }
