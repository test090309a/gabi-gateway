...
def _classify_intent_enhanced(user_message: str) -> Dict[str, Any]:
    """
    Erweiterte Intent-Erkennung - mit PRIORITÄT FÜR CHAT!
    """
    msg = user_message.lower().strip()
    
    # ===== PRIO 1: DIREKTE CHAT-FRAGEN (MÜSSEN ZUERST KOMMEN!) =====
    chat_question_patterns = [
        # Persönliche Fragen
        (r"(?:wie|wer|was)\s+ist\s+(?:dein|ihr)\s+name", "chat"),
        (r"wer\s+(?:bist|sind)\s+du", "chat"),
        (r"wie\s+heißt\s+du", "chat"),
        (r"was\s+kannst\s+du", "chat"),
        (r"was\s+ist\s+deine\s+aufgabe", "chat"),
        (r"erzähl\s+mir\s+(?:etwas|was)", "chat"),
        
        # Begrüßungen
        (r"^(hallo|hi|hey|servus|moin|guten tag|guten morgen)$", "chat"),
        (r"wie\s+geht\s+(?:es|'s)", "chat"),
    ...
    for pattern, intent in chat_question_patterns:
        if re.search(pattern, msg, re.IGNORECASE):
            logger.info(f"💬 Chat-Frage erkannt (Pattern): {user_message[:50]}...")
            return {
                "intent": "chat",
                "confidence": 0.95,
                "method": "chat_pattern",
    ...
    # ===== PRIO 3: EXPLIZITE SUCH-ANFRAGEN =====
    ...
    # ===== PRIO 5: SEMANTISCHE ERKENNUNG mit LOWER THRESHOLD =====
    try:
        semantic_result = _classify_intent_semantic(user_message)
        
        # Korrektur: Wenn semantisch "som_search" erkannt wird, aber KEINE Such-Keywords vorhanden sind
        if semantic_result.get("intent") == "som_search":
            # Prüfe ob es wirklich ein Such-Befehl ist
            has_search_keyword = any(kw in msg for kw in explicit_search_keywords)
            has_question_mark = "?" in msg
            is_simple_question = len(msg.split()) <= 8 and has_question_mark
            
            # Wenn es eine einfache Frage ohne Such-Keyword ist -> Chat!
            if not has_search_keyword and is_simple_question:
                logger.info(f"⚠️ Korrigiere falsche som_search-Erkennung → chat")
                return {
                    "intent": "chat",
                    "confidence": 0.85,
                    "method": "semantic_corrected",
                    "original_intent": semantic_result.get("intent")
                }
        
        if semantic_result.get("confidence", 0) > 0.6:  # Etwas niedrigerer Threshold
