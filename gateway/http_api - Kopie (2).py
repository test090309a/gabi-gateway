...
@router.post("/shell")
async def execute_command(command: str, args: list = [], token: str = Header(None)):
    ...
    if token != config.get("api_key"):
        raise HTTPException(status_code=403, detail="Access Denied: Falscher API-Key")

    allowed_commands = config.get("shell.allowed_commands", [])
    
    # Sicherheitscheck: Ist der Befehl in der Liste erlaubt?
    if command not in allowed_commands:
        raise HTTPException(status_code=400, detail=f"Befehl '{command}' ist nicht freigegeben!")

    try:
        # Wir bauen den Befehl zusammen
        # Unter Windows hilft shell=True bei internen Befehlen wie 'dir' oder 'echo'
        ...
        result = subprocess.run(
            ...
            text=True, 
            shell=True, 
            timeout=10 # Verhindert, dass das Gateway hängen bleibt,
            encoding='cp850' # Hilft gegen die komischen Zeichen wie 'â' in der CMD-Ausgabe
        )
