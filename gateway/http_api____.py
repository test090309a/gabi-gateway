        ...
        if intent_result.get("intent", "").startswith("som_"):
            ...
            if intent == "som_search":
                ...
                try:
                    ...
                    logger.error(f"SoM Agent Fehler: {e}")
                    result = {"success": False, "error": str(e)}
                finally:
                    agent.close()
                    logger.info("🧹 SoM Agent Browser geschlossen")
                
                if result and result.get("success"):
                    extracted = result.get("extracted_content", {})
                    raw_results = extracted.get("search_results", [])
                    
                    if raw_results:
                        _progress_add(request_id, f"🤖 Analysiere {len(raw_results)} Suchergebnisse...", "fa-brain")
                        
                        # ===== INTELLIGENTE MODELL-AUSWAHL BASIEREND AUF QUERY-TYP =====
                        query_lower = query.lower()
                        
                        # Erkenne Query-Typ
                        is_code = any(word in query_lower for word in [
                            'code', 'python', 'javascript', 'html', 'css', 'programmieren', 
                            'skript', 'funktion', 'api', 'json', 'sql', 'algorithmus'
                        ])
                        
                        is_math = any(word in query_lower for word in [
                            'berechnung', 'formel', 'gleichung', 'mathe', 'mathematik',
                            'summe', 'durchschnitt', 'prozent', 'wurzel', 'integral'
                        ])
                        
                        is_technical = is_code or is_math or any(word in query_lower for word in [
                            'grafikkarte', 'prozessor', 'hardware', 'software', 'linux', 
                            'windows', 'server', 'docker', 'kubernetes', 'git'
                        ])
                        
                        is_weather = any(word in query_lower for word in [
                            'wetter', 'temperatur', 'regen', 'sonne', 'vorhersage', 'grad'
                        ])
                        
                        is_simple = len(query.split()) < 5 and not is_technical and not is_weather
                        
                        # Verfügbare Modelle abrufen
                        models_info = ollama_client.list_models()
                        available = [m.get("name") for m in models_info.get("models", []) if m.get("name")]
                        
                        # Modell-Auswahl nach Typ
                        if is_code:
                            # Code-Fragen: Coder-Modelle bevorzugen
                            preferred = ["qwen2.5-coder:7b", "qwen2.5-coder:14b", "deepseek-coder:6.7b", "codellama:latest"]
                            extraction_model = None
                            for pref in preferred:
                                if pref in available:
                                    extraction_model = pref
                                    break
                            if not extraction_model:
                                extraction_model = "qwen2.5:7b" if "qwen2.5:7b" in available else "llama3.1:8b"
                            logger.info(f"🎯 Code-Query erkannt, verwende Coder-Modell: {extraction_model}")
                            _progress_add(request_id, f"💻 Code-Frage erkannt → {extraction_model}", "fa-code")
                            
                        elif is_technical:
                            # Technische Fragen: Starke, präzise Modelle
                            preferred = ["qwen2.5:7b", "qwen3:8b", "llama3.1:8b", "mistral:7b-instruct-v0.3-q5_K_M"]
                            extraction_model = None
                            for pref in preferred:
                                if pref in available:
                                    extraction_model = pref
                                    break
                            if not extraction_model:
                                extraction_model = "qwen2.5:14b" if "qwen2.5:14b" in available else "llama3.1:8b"
                            logger.info(f"🔧 Technische Query erkannt, verwende: {extraction_model}")
                            _progress_add(request_id, f"🔧 Technische Frage → {extraction_model}", "fa-microchip")
                            
                        elif is_weather:
                            # Wetter-Fragen: Schnelles, präzises Modell
                            preferred = ["qwen2.5:7b", "llama3.1:8b", "qwen3:8b"]
                            extraction_model = None
                            for pref in preferred:
                                if pref in available:
                                    extraction_model = pref
                            ...
                            logger.info(f"🌤️ Wetter-Query erkannt, verwende: {extraction_model}")
                            _progress_add(request_id, f"🌤️ Wetter-Frage → {extraction_model}", "fa-sun")
                            
                        elif is_simple:
                            # Einfache Fragen: Schnelles Modell
                            preferred = ["qwen2.5:7b", "qwen2.5:1.5b", "llama3.2:3b"]
                            extraction_model = None
                            for pref in preferred:
                                if pref in available:
                                    extraction_model = pref
                                    break
                            if not extraction_model:
                                extraction_model = "qwen2.5:7b" if "qwen2.5:7b" in available else "qwen2.5:1.5b"
                            logger.info(f"⚡ Einfache Query erkannt, verwende schnelles Modell: {extraction_model}")
                            _progress_add(request_id, f"⚡ Einfache Frage → {extraction_model}", "fa-bolt")
                            
                        else:
                            # Komplexe Allgemeinfragen: Stärkstes verfügbares Modell
                            strong_models = ["qwen2.5:7b", "qwen3:8b", "llama3.1:8b", "mistral:7b-instruct-v0.3-q5_K_M"]
                            extraction_model = None
                            for strong in strong_models:
                                if strong in available:
                                    extraction_model = strong
                                    break
                            if not extraction_model:
                                extraction_model = "qwen2.5:7b" if "qwen2.5:7b" in available else "llama3.1:8b"
                            logger.info(f"📚 Komplexe Query erkannt, verwende starkes Modell: {extraction_model}")
                            _progress_add(request_id, f"📚 Komplexe Frage → {extraction_model}", "fa-brain")
                        
                        _progress_set_active_model(request_id, extraction_model)
                        
                        # Formatiere Suchergebnisse für LLM mit Quellen-IDs
                        search_context = ""
                        for i, res in enumerate(raw_results[:6], 1):  # Max 6 für präzisen Kontext
            ...
            ---"""
                        ...
                        # ===== EXTRAKTIONS-PROMPT (angepasst an Query-Typ) =====
                        if is_code:
                            extraction_prompt = f"""Du bist ein präziser Code- und Technik-Experte.
            ...
            **REGELN:**
            1. Beantworte die Frage AUSSCHLIESSLICH mit Informationen aus den Quellen oben
            2. Wenn Code-Beispiele in den Quellen sind, gib sie korrekt wieder
            3. Markiere jede Information mit der Quellen-Nummer in Klammern, z.B. (Quelle 1)
            4. ERFINDE NICHTS – keine Code-Snippets, Funktionen oder Syntax erfinden
            5. Antworte auf Deutsch, klar und technisch präzise

            **ANTWORT (nur basierend auf den Quellen):**"""
                        
                        elif is_weather:
                            extraction_prompt = f"""Du bist ein präziser Wetter-Experte.
            ...
            **REGELN:**
            1. Beantworte die Frage AUSSCHLIESSLICH mit Informationen aus den Quellen oben
            2. Gib Temperaturen und Wetterbedingungen NUR an, wenn sie explizit in den Quellen stehen
            3. Markiere jede Information mit der Quellen-Nummer in Klammern, z.B. (Quelle 1)
            4. ERFINDE KEINE Temperaturen, Wetterbedingungen oder Zeitangaben
            5. Antworte auf Deutsch, klar und verständlich
            ...
            **REGELN (UNBEDINGT BEFOLGEN!):**
            1. Beantworte die Frage AUSSCHLIESSLICH mit Informationen aus den Quellen oben
            2. Wenn die Antwort nicht in den Quellen steht, antworte: "Keine Informationen gefunden"
            3. Markiere jede Information mit der Quellen-Nummer in Klammern, z.B. (Quelle 1)
            4. ERFINDE NICHTS – keine Namen, Daten, Fakten oder Details erfinden
            5. Wenn du eine konkrete Zahl, ein Datum oder einen Namen nennst, muss er EXPLIZIT in den Quellen stehen
            6. Antworte auf Deutsch, klar und verständlich
            ...
            **ANTWORT (nur basierend auf den Quellen):**"""
                        ...
                        try:
                            ...
                            if "Keine Informationen gefunden" in summary or len(summary) < 20:
                                summary = f"Ich habe folgende Informationen gefunden:\n\n"
                            ...
                            # Erstelle finale Antwort mit Quellen
                            reply = f"🔍 **{query}**\n\n"
                            reply += f"{summary}\n\n"
                            reply += f"---\n📚 **Quellen:**\n"
                            for i, res in enumerate(raw_results[:5], 1):
                                title = res.get('title', 'Kein Titel')[:60]
                                url = res.get('url', '')
                                reply += f"{i}. [{title}]({url})\n"
                            ...
                            return {
                                ...
                                "tool_used": "som_search_extracted",
                                "model_used": extraction_model,
                                "query_type": "code" if is_code else "technical" if is_technical else "weather" if is_weather else "general",
                            ...
                            # Fallback: Nur rohe Ergebnisse
                            reply = f"🔍 **Suchergebnisse für '{query}':**\n\n"
                            for i, res in enumerate(raw_results[:10], 1):
                                title = res.get('title', 'Kein Titel')
                                url = res.get('url', '')[:80]
                                reply += f"{i}. **{title}**\n"
                                reply += f"   🔗 {url}\n\n"
                    ...
                    error_msg = result.get('error') if result else "Unbekannter Fehler"
                    return {
                        "status": "error",
                        "reply": f"❌ Suche fehlgeschlagen: {error_msg}",
                        "request_id": request_id
                    }
...
"""
    ...
    **💡 TIPPS:**
    ...
    # ===== FOLGEFRAGEN =====
    # Prüfe ob eine sinnvolle Antwort vorliegt (nicht "Keine Informationen" und nicht zu kurz)
    if summary and "Keine Informationen" not in summary and len(summary) > 50:
        try:
            # Wähle das richtige Modell für Folgefragen (klein und schnell)
            available_models = [m.get("name") for m in models_info.get("models", []) if m.get("name")]
            
            # Bevorzugte Modelle für Folgefragen (schnell)
            preferred_followup = ["qwen2.5:7b", "qwen2.5:1.5b", "llama3.2:3b"]
            followup_model = None
            for pref in preferred_followup:
                if pref in available_models:
                    followup_model = pref
                    break
            
            if not followup_model:
                followup_model = _pick_fast_model(available_models) or "qwen2.5:1.5b"
            
            # Begrenze die Antwort für den Prompt (max 400 Zeichen)
            answer_snippet = summary[:400] if len(summary) > 400 else summary
            
            followup_prompt = f"""Du bist ein hilfreicher Assistent. Basierend auf der folgenden Antwort, generiere 2-3 sinnvolle Folgefragen, die der Nutzer als nächstes stellen könnte.

    **ANTWORT:**
    {answer_snippet}

    **REGELN:**
    - Die Fragen sollen natürlich und hilfreich sein
    - Sie sollen thematisch zur Antwort passen
    - Vermeide Fragen, die bereits beantwortet wurden
    - Antworte NUR mit einem JSON-Array, z.B.: ["Frage 1", "Frage 2", "Frage 3"]

    **FOLGEFRAGEN (JSON-Array):**"""

            followup_response = await _ollama_chat_async(
                model=followup_model,
                messages=[{"role": "user", "content": followup_prompt}],
                options={"temperature": 0.6, "num_predict": 120}
            )
            followup_text = _extract_ollama_text(followup_response)
            ...
            if json_match:
                try:
                    followup_questions = json.loads(json_match.group())
                    if isinstance(followup_questions, list) and len(followup_questions) > 0:
                        # Begrenze auf max 3 Fragen
                        followup_questions = followup_questions[:3]
                        
                        reply += "\n\n---\n💡 **Weitere Fragen:**\n"
                        for q in followup_questions:
                            # Entferne eventuelle Anführungszeichen und clean up
                            q = q.strip('"\'')
                            reply += f"• {q}\n"
                        
                        logger.info(f"💡 Folgefragen generiert: {followup_questions}")
                except json.JSONDecodeError:
                    logger.debug(f"JSON-Parsing fehlgeschlagen: {followup_text[:100]}")
                    
        except Exception as e:
            logger.debug(f"Folgefragen fehlgeschlagen: {e}")
