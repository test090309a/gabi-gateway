...
# Integration des Gehirn-Konzepts
...
async def chat_with_gabi(request: ChatRequest, token: str = Header(None)):
    """GABI nutzt jetzt ihr volles Gehirn!"""
    
    brain = get_brain()
    
    # Bestimme Typ der Anfrage
    task = {
        "content": request.message,
        "type": request.type if hasattr(request, 'type') else "auto"
    }
    
    # Verarbeite mit dem Corpus Callosum
    result = brain.route_task(task)
    
    return {
        "status": "success",
        "reply": result.get("combined_response", result.get("response", "🤔")),
        "hemisphere_used": result.get("hemisphere", "bridge"),
        "timestamp": datetime.now().isoformat()
    }

@router.get("/brain/status")
async def brain_status(_api_key: str = Depends(verify_api_key)):
    """Zeigt Status von GABIs Gehirnhälften"""
    brain = get_brain()
