...
def register_builtin_skills() -> None:
    """Registriert eingebaute Skills wie SoM-Agent."""
    memory = get_memory()

    # SoM Web-Automation Skill
    if not memory.has_skill("som_agent"):
        memory.add_skill(
            skill_name="som_agent",
            requirement="Universelle Web-Automation mit Set-of-Mark (SoM) und Vision-KI",
            libraries=["selenium", "webdriver-manager", "ollama"],
            security_score=95
        )
        logger.info("SoM-Agent Skill registriert")
