...
class OllamaClient:
    ...
    def __init__(self):
        ...
        self.default_model = config.get("ollama.default_model", "llama2:latest") # granite4:tiny-h
        try:
    ...
    def embeddings(self, model: str, prompt: str) -> dict:
        """
        Erzeugt Embeddings für einen Text über die Ollama API.
        
        Args:
            model: Das Embedding-Modell (z.B. 'nomic-embed-text')
            prompt: Der zu embeddende Text
            
        Returns:
            Dict mit 'embedding' Liste
        """
        payload = {
            "model": model,
            "prompt": prompt
        ...
        logger.debug(f"Embeddings request | model={model} | prompt_len={len(prompt)}")
        ...
        try:
            ...
            logger.debug(f"Embeddings response | dimension={len(embedding)}")
            return data
        except httpx.HTTPError as e:
            logger.error(f"Ollama embeddings error: {e}")
            if isinstance(e, httpx.ReadTimeout):
                raise RuntimeError(
                    f"Ollama embeddings request timed out after {int(self.timeout_seconds)}s. "
                    "Nutze ein kleineres Modell oder erhöhe ollama.timeout_seconds."
                )
