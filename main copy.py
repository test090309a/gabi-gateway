...
async def lifespan(app: FastAPI):
    ...
    # SoM Agent für Web-Automation registrieren
    try:
        from gateway.memory_extensions import register_builtin_skills
        register_builtin_skills()
        logger.muted("SoM-Agent: Skill registriert")
    except Exception as e:
        logger.warning(f"SoM-Agent: Registrierung fehlgeschlagen - {e}")
...
app = FastAPI(
    title="GABI Gateway",
    description="Gateway mit lokaler LLM, Telegram-Bot, Gmail, Whisper, ComfyUI und Shell Befehlen",
    version="1.0.2",
    lifespan=lifespan,
)
