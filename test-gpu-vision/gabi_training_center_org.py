...
class GabiTrainingCenter:
    ...
    def run_manual_training(self, module):
        ...
        for item in self.elements_tree.get_children():
            ...
            elements.append(values[0])  # Element-Name
        ...
        for i, element in enumerate(elements):
            ...
            self.elements_tree.item(self.elements_tree.get_children()[i], values=(element, hotkey, "🟡 wartet"))
            ...
            self.elements_tree.item(self.elements_tree.get_children()[i], values=(element, hotkey, "✅ erfasst"))
            
            self.train_log_message(f"✅ {element} erfasst bei ({x}, {y})")
    ...
    def setup_recipe_automation(self, parent):
        ...
        ttk.Label(left_frame, text="📚 Verfügbare Rezepte:").pack(anchor='w')
        ...
        ttk.Button(btn_frame, text="▶️ Rezept ausführen", command=self.run_recipe).pack(side='left', padx=5)
        ttk.Button(btn_frame, text="💾 Speichern", command=self.save_recipe).pack(side='left', padx=5)
        ttk.Button(btn_frame, text="🗑️ Löschen", command=self.delete_recipe).pack(side='left', padx=5)
    ...
    def setup_scheduler_automation(self, parent):
        ...
        ttk.Button(control_frame, text="➕ Job hinzufügen", command=self.add_scheduled_job).pack(side='left', padx=5)
        ttk.Button(control_frame, text="▶️ Scheduler starten", command=self.start_scheduler).pack(side='left', padx=5)
        ttk.Button(control_frame, text="⏹️ Scheduler stoppen", command=self.stop_scheduler).pack(side='left', padx=5)
